import React, { useState, useEffect } from 'react'

export default function WordFrequencyCounter() {
  const [text, setText] = useState('')
  const [wordFrequency, setWordFrequency] = useState({})
  const [totalWords, setTotalWords] = useState(0)

  useEffect(() => {
    const words = text.toLowerCase().match(/\b[\w']+\b/g) || []
    const frequency = {}
    words.forEach((word) => {
      frequency[word] = (frequency[word] || 0) + 1
    })
    setWordFrequency(frequency)
    setTotalWords(words.length)
  }, [text])

  const downloadCSV = () => {
    const csvContent = Object.entries(wordFrequency)
      .sort(([, a], [, b]) => b - a)
      .map(([word, count]) => `${word},${count},${((count / totalWords) * 100).toFixed(2)}%`)
      .join('\n')
    const blob = new Blob(['Word,Count,Percentage\n' + csvContent], { type: 'text/csv;charset=utf-8;' })
    const link = document.createElement('a')
    if (link.download !== undefined) {
      const url = URL.createObjectURL(blob)
      link.setAttribute('href', url)
      link.setAttribute('download', 'word_frequency.csv')
      link.style.visibility = 'hidden'
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
    }
  }

  return (
    <div className="min-h-screen bg-white text-black font-sans relative">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold mb-8 text-center">Word Frequency Counter</h1>
        <div className="max-w-2xl mx-auto">
          <textarea
            value={text}
            onChange={(e) => setText(e.target.value)}
            placeholder="Paste your text here..."
            className="w-full p-4 mb-4 border border-black text-lg resize-none h-48 focus:outline-none focus:ring-2 focus:ring-black"
          />
          {totalWords > 0 && (
            <div className="mt-8">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold">Results</h2>
                <button
                  onClick={downloadCSV}
                  className="bg-black text-white px-4 py-2 text-sm font-bold hover:bg-gray-800 transition-colors"
                >
                  Download CSV
                </button>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div className="font-bold">Word</div>
                <div className="font-bold text-center">Count</div>
                <div className="font-bold text-right">Percentage</div>
                {Object.entries(wordFrequency)
                  .sort(([, a], [, b]) => b - a)
                  .map(([word, count]) => (
                    <React.Fragment key={word}>
                      <div className="truncate">{word}</div>
                      <div className="text-center">{count}</div>
                      <div className="text-right">{((count / totalWords) * 100).toFixed(2)}%</div>
                    </React.Fragment>
                  ))}
              </div>
            </div>
          )}
        </div>
      </div>
      <a
        href="https://github.com/ifalfahri/wordfreq"
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-4 right-4 bg-black text-white p-2 rounded-full hover:bg-gray-800 transition-colors"
        aria-label="View source on GitHub"
      >
        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
        </svg>
      </a>
    </div>
  )
}